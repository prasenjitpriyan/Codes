...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/dataModel.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getCompletedList = exports.getShoppingList = exports.setPriority = exports.addToCompletedList = exports.addToShoppingList = void 0;
let shoppingList = [];
let completedList = [];

const addToShoppingList = task => {
  const taskId = `${parseInt(Math.random() * 100000000)}-${new Date().getTime()}`;
  shoppingList.push({
    task,
    id: taskId,
    priority: 'normal'
...
const addToCompletedList = taskId => {
  const getTask = shoppingList.find(({
    id
  }) => id === taskId);
  shoppingList = shoppingList.filter(({
    id
  }) => id !== taskId);
  completedList = [getTask, ...completedList];
};
...
const Task = (title, priority = 'normal', id) => {
...
const renderShoppingList = () => {
  const domNodes = (0, _dataModel.getShoppingList)().map(item => {
    return (0, _Task.default)(item.task, item.priority, item.id);
  });
...
const renderCompletedList = () => {
  const domNodes = (0, _dataModel.getCompletedList)().map(item => {
    return (0, _Task.default)(item.task, item.priority, item.id);
  });
...
},{"./dataModel":"js/dataModel.js","./Task":"js/Task.js"}],"js/index.js":[function(require,module,exports) {
"use strict";
...
const shoppingListDiv = document.querySelector('.shopping-list');
const completedDiv = document.querySelector('.completed');
const itemEl = document.querySelector('.shopping-list > .item');
let shoppingList = [];
let completedList = []; // const renderShoppingList = () => {
//   const domNodes = shoppingList.map((item) => {
//     return Task(item.task, item.priority, item.id);
//   });
//   console.log(shoppingList);
//   shoppingListDiv.innerHTML = domNodes.join('');
// };
// const renderCompletedList = () => {
//   const domNodes = completedList.map((item) => {
//     return Task(item.task, item.priority, item.id);
//   });
...
taskInput.addEventListener('keyup', function (evt) {
  if (evt.key === 'Enter') {
    (0, _dataModel.addToShoppingList)(evt.target.value);
...
shoppingListDiv.addEventListener('click', function (evt) {
  if (evt.target.parentElement.classList.contains('priority-control')) {
    ...
    const taskId = evt.target.parentElement.parentElement.getAttribute('data-id');
    (0, _dataModel.setPriority)(taskId, priority);
    (0, _view.renderShoppingList)();
  }

  if (evt.target.classList.contains('status')) {
    const taskId = evt.target.parentElement.getAttribute('data-id');
    console.log(taskId);
  }
});
...
completedDiv.addEventListener('drop', function (evt) {
  ...
  const getTask = shoppingList.filter(item => item.id === taskId)[0];
  completedList = [getTask, ...completedList];
  shoppingList = shoppingList.filter(item => item.id !== taskId);
  (0, _view.renderShoppingList)();
  renderCompletedList();
});
...
},{"./dataModel":"js/dataModel.js","./view":"js/view.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
